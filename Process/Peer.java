...
public class Peer {
    ...
    private static void readConfiguration() throws IOException {
        ...
        Files.lines(Paths.get("Configs/Common.cfg")).forEach(line -> {
            ...
            switch (p[0]) {
                case "NumberOfPreferredNeighbors":
                    SysConfig.preferredNeighbourCount = Integer.parseInt(p[1]);
                    break;
                case "UnchokingInterval":
                    SysConfig.unchokingInterval = Integer.parseInt(p[1]);
                    break;
                case "OptimisticUnchokingInterval":
                    SysConfig.optimisticUnchokingInterval = Integer.parseInt(p[1]);
                    break;
                case "FileSize":
                    SysConfig.fileSize = Integer.parseInt(p[1]);
                    break;
                case "FileName":
                    SysConfig.fileName = p[1];
                    break;
                case "PieceSize":
                    SysConfig.pieceSize = Integer.parseInt(p[1]);
                    break;
            }
        });
    
        // *now* log what you loaded:
        Helper.tag("CONFIG", String.format(
            "Loaded Common.cfg → NP=%d, p=%d, m=%d, file=%s, size=%d, piece=%d",
            SysConfig.preferredNeighbourCount,
            SysConfig.unchokingInterval,
            SysConfig.optimisticUnchokingInterval,
            SysConfig.fileName,
            SysConfig.fileSize,
            SysConfig.pieceSize
        ));
        ...
        // and only *after* populating the map:
        Helper.tag("CONFIG", "Loaded " + peerLines.size() + " entries from PeerInfo.cfg");
    }
